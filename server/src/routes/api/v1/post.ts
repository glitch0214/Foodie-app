...
router.get(
	...
	async (req: Request, res: Response, next: NextFunction) => {
		try {
			const redisClient =
				await require("../../../config/redis").getConnection();
			const redisKey = `${req.params.username}_posts`;
			...
			if (cachedResponse) {
				console.log(
					`using cache: user feed | username: ${req.params.username}`
				);
			...
			await redisClient.set(redisKey, JSON.stringify(makeResponseJson(agg)), {
				EX: 60,
				NX: true,
			});
			console.log("Set cache done ...");
